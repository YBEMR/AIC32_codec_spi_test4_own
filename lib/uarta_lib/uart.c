...
// Send 16-bit data (Big Endian: MSB first)
void UARTa_SendUint16(Uint16 data) {
    Uint16 msb = (data >> 8) & 0xFF;  // High byte
    Uint16 lsb = data & 0xFF;         // Low byte

    UARTa_SendByte(msb);  // Send high byte first
    UARTa_SendByte(lsb);  // Send low byte second
}


// Custom function to send a number (convert to string)
void UARTa_SendNumber(int32 number) {
    char buf[16];  // Buffer, sufficient size (max 5 digits + \n)
    sprintf(buf, "%ld", number);  // Format as string (e.g., "1234")
...
// Send hexadecimal number (with "0x" prefix, uppercase)
void UARTa_SendHex(Uint16 number) {
    char buf[16];  // Buffer: enough for "0xFFFF\n" (7 characters)
    sprintf(buf, "0x%02X", number);  // Format: 0xABCD (%04X zero padding, uppercase)
...
void UARTa_SendStringAndNumber(char * msg1, int32 number, char * msg2){
    UARTa_SendString(msg1);
    UARTa_SendNumber(number);
    UARTa_SendString(msg2);
}

void UARTa_SendStringAndHex(char * msg1, Uint16 number, char * msg2){
    UARTa_SendString(msg1);
    UARTa_SendHex(number);
    UARTa_SendString(msg2);
}


// Used for baud rate auto-detection test
