...
**************************************************************************
*/
void Decoder_amr_reset (Decoder_amrState *state, Flag dtx_partial_reset)
{
  Word16 i;
...
//          printf("%d %d %d\n, ", *(st->exc), *(st->exc+1) ,*(st->exc+2));
        ...
        *-------------------------------------------------------*/
        ...
        if (sub (pit_sharp, 16384) > 0)
        {
           for (i = 0; i < L_SUBFR; i++)
            {
               temp = mult (st->exc[i], pit_sharp);  //temp都不对，pit_sharp是对的，st->exc[i]错的
//               printf("%d ", pit_sharp);
